# Workspace Detection
*Source: [awslabs/aidlc-workflows](https://github.com/awslabs/aidlc-workflows) — `aidlc-rules/aws-aidlc-rule-details/inception/workspace-detection.md`*
...
- **IF reverse engineering artifacts exist**:
    - Check if artifacts are stale (compare timestamps against codebase's last significant modification)
    - **IF artifacts are current**: Load them, skip to Requirements Analysis
    - **IF artifacts are stale**: Next phase is Reverse Engineering
    - **IF user explicitly requests rerun**: Next phase is Reverse Engineering
...
- Automatically proceed to next phase:
  - **Brownfield**: Reverse Engineering (or Requirements Analysis if current artifacts exist)
